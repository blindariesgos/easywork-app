    ...
    <div className="w-full  text-black  rounded-lg px-4 lg:px-8">
      ...
      <Disclosure as="div" className="rounded-lg bg-gray-100" defaultOpen={true}>
        ...
        <DisclosurePanel className="mt-2 px-8  py-2 overflow-y-auto max-h-[700px]">
          {folders.map(folder => (
            <Link href={`/tools/drive?folder=${folder.id}`} key={folder.name} className="flex items-center gap-x-3 cursor-pointer">
              <PiCheckFatFill className="text-primary w-4 h-4" />
              <p className="py-3">{folder?.metadata?.observableName ?? folder.name}</p>
            </Link>
          ))}
        </DisclosurePanel>
      </Disclosure>
