...
export default function General({ agent, id, refPrint, type, handleAdd }) {
  ...
  useEffect(() => {
    ...
    if (session && typeof agent === 'undefined') {
      setValue('recruitmentManagerId', lists?.users?.find(user => user.id === session?.user?.sub)?.id);
    }
  ...
  useEffect(() => {
    ...
    if (agent?.user?.profile?.firstName) setValue('firstName', agent?.user?.profile?.firstName);
    if (agent?.user?.profile?.lastName) setValue('lastName', agent?.user?.profile?.lastName);
    ...
    if (agent?.user?.profile?.idcard) setValue('rfc', agent?.user?.profile?.idcard);
    if (agent?.user?.bio) setValue('bio', agent?.user?.bio);
    if (agent?.address) setValue('address', agent?.address);
    if (agent?.recruitmentManager) setValue('recruitmentManagerId', agent?.recruitmentManager?.id);
    if (agent?.developmentManager) setValue('developmentManagerId', agent?.developmentManager?.id);
    ...
    if (type === 'recruitment') {
      ...
      if (recruitment?.startDate) setValue('recruitmentStartDate', moment(recruitment?.startDate).subtract(utcOffset, 'minutes').format());
      if (recruitment?.endDate) setValue('recruitmentEndDate', moment(recruitment?.endDate).subtract(utcOffset, 'minutes').format());
      if (recruitment?.entryDate) setValue('recruitmentEntryDate', moment(recruitment?.entryDate).subtract(utcOffset, 'minutes').format());
      if (recruitment?.agentRecruitmentStage) setValue('agentRecruitmentStageId', recruitment?.agentRecruitmentStage?.id);
    }
    if (type === 'conection') {
      const connection = agent?.connections[0] ?? {};

      if (connection?.endDate) setValue('connectionEndDate', moment(connection?.endDate).subtract(utcOffset, 'minutes').format());
      if (connection?.startDate) setValue('connectionStartDate', moment(connection?.startDate).subtract(utcOffset, 'minutes').format());
      if (connection?.idcardNumber) setValue('idcardNumber', connection?.idcardNumber);
      if (connection?.cnsfDate) setValue('connectionCnsfDate', moment(connection?.cnsfDate).subtract(utcOffset, 'minutes').format());
      if (connection?.effectiveDateCua) setValue('effectiveDateCua', moment(connection?.effectiveDateCua).subtract(utcOffset, 'minutes').format());
      if (connection?.effectiveDateIdcard) setValue('effectiveDateIdcard', moment(connection?.effectiveDateIdcard).subtract(utcOffset, 'minutes').format());
      if (connection?.agentConnectionStage) setValue('agentConnectionStageId', connection?.agentConnectionStage?.id);
    }
    setLoading(false);
  }, [agent, id]);
    ...
    <Fragment>
      <div className="px-4 lg:px-8 h-full w-full">
        <form
          ...
          className={clsx('grid grid-cols-1 lg:h-full bg-gray-100 rounded-lg  w-full', {
        ...
        >
          ...
          <div className=" bg-gray-100 p-4 lg:overflow-y-scroll rounded-lg lg:col-span-5 ">
            <div className="flex justify-between bg-white py-4 px-3 rounded-md">
              <h1 className="">{t('agentsmanagement:accompaniments:agent:data')}</h1>
              {agent && (
                <button type="button" disabled={!id} onClick={() => setIsEdit(!isEdit)} title="Editar">
            ...
            <div className="flex justify-center">
              {isEdit ? (
                <ProfileImageInput selectedProfileImage={selectedProfileImage} onChange={handleProfileImageChange} disabled={!isEdit} />
              ) : (
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-3 pb-[7rem] pt-4">
              ...
              <Controller
                render={({ field: { ref, ...field } }) => {
                  return <InputPhone name="phone" field={field} error={errors.phone} label={t('contacts:create:phone')} defaultValue={field.value} disabled={!isEdit} />;
                }}
                name="phone"
                control={control}
                defaultValue=""
              />
              <MultiplePhonesInput errors={errors.otherPhones} register={register} name="otherPhones" disabled={!isEdit} control={control} watch={watch} setValue={setValue} />
              <TextInput label={t('contacts:create:email')} placeholder={t('contacts:create:placeholder-lastname')} error={errors.email} register={register} name="email" disabled={!isEdit} />

              <MultipleEmailsInput errors={errors.otherEmails} register={register} name="otherEmails" disabled={!isEdit} control={control} watch={watch} setValue={setValue} />
              {/* {!isEdit && (
                <TextInput
                ...
                render={({ field: { value, onChange, ref, onBlur } }) => {
                  return (
                    <InputDate
                      ...
                      onChange={onChange}
                      onBlur={onBlur}
                      icon={<FaCalendarDays className="h-3 w-3 text-primary pr-4 mr-2" />}
              ...
              <TextInput label={t('agentsmanagement:accompaniments:agent:children')} error={errors?.childrens} register={register} name="childrens" type="number" disabled={!isEdit} />
              <AddressInput
              ...
              {type === 'conection' && (
                <Fragment>
                  <SelectInput
                    ...
                    watch={watch}
                  />
                  <TextInput label={t('agentsmanagement:conections:idcardNumber')} error={errors?.idcardNumber} register={register} name="idcardNumber" disabled={!isEdit} />
                  <Controller
                    render={({ field: { value, onChange, ref, onBlur } }) => {
                      return (
                        <InputDate
                          label={t('agentsmanagement:conections:effectiveDateCua')}
                          value={value}
                          onChange={onChange}
                          onBlur={onBlur}
                          icon={<FaCalendarDays className="h-3 w-3 text-primary pr-4 mr-2" />}
                  ...
                  <Controller
                    render={({ field: { value, onChange, ref, onBlur } }) => {
                      return (
                        <InputDate
                          label={t('agentsmanagement:conections:effectiveDateIdcard')}
                          value={value}
                          onChange={onChange}
                          onBlur={onBlur}
                          icon={<FaCalendarDays className="h-3 w-3 text-primary pr-4 mr-2" />}
              ...
              <UserSelectAsync
                ...
                watch={watch}
              />
              <UserSelectAsync label={t('agentsmanagement:accompaniments:agent:observer')} name="observerId" disabled={!isEdit} error={errors?.observerId} setValue={setValue} watch={watch} />
          ...
          {/* Menu Derecha */}
          {id && agent && <ActivityPanel entityId={id} crmType="agent" className="lg:col-span-7" />}
          {/* Botones de acción */}
          {(isEdit || !agent) && (
            <div
              className={clsx('flex justify-center px-4 w-full py-4 gap-4 bottom-0 lg:rounded-bl-[35px] rounded-none left-0 right-0 fixed lg:absolute bg-white shadow-[0px_-2px_6px_4px_#00000017] ')}
            >
              <Button type="submit" label={loading ? t('common:buttons:saving') : t('common:buttons:save')} disabled={loading} buttonStyle="primary" className="px-3 py-2" />
              <Button type="button" label={t('common:buttons:cancel')} disabled={loading} buttonStyle="secondary" onclick={() => router.back()} className="px-3 py-2" />
            </div>
          )}
        </form>
      </div>
