...
export const AssignCourseModal = ({ course, isOpen, setIsOpen, onSuccess }) => {
  ...
  const onSave = async () => {
    try {
      const userToAssign = agents?.items.filter(
        ({ user }) => user && isUserSelected(user.id)
      );
      ...
      router.push(
        `/tools/tasks/task?show=true&prev=course-assign&prev_id=${course.id}`
      );
    } catch (error) {
      console.log(error);
  ...
  const fetchCourseDetails = useCallback(async () => {
    ...
    try {
      ...
      setUsersAlreadyAssigned(
        assignedUsers.map((assignedUser) => assignedUser.user.id)
      );
    } catch (error) {
      toast.error(
        "Estamos teniendo problemas para obtener el detalle del curso a asignar. Por favor intenta más tarde"
      );
    }
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-2 ">
        <DialogPanel className="min-w-96 md:w-[700px] p-6 rounded-xl bg-gray-100">
          <DialogTitle className="text-lg">
            Curso:{" "}
            <span className="text-easy-400 font-bold">{course?.name}</span>
          </DialogTitle>
          ...
          <div className="mb-5">
            <p className="text-sm">
              Personas ({agents?.meta.totalItems || agents?.items.length || 0})
            </p>
