...
import { createCourse } from '../courses/services/create-course';
...
export default function NewCourseModal() {
  ...
  const catchErrorOnSubmit = async errors => {
    console.log('🚀 ~ catchErrorOnSubmit ~ errors:', errors);
  };
      ...
      <Dialog open={isOpen} onClose={onCloseModal} className="relative z-50">
        ...
        <div className="fixed inset-0 flex w-screen items-center justify-center p-2 ">
          ...
          <DialogPanel className="min-w-96 space-y-8  p-6 rounded-xl bg-gray-100">
            ...
            <form action={handleSubmit(onSubmit, catchErrorOnSubmit)}>
              <NewCourseForm register={register} setValue={setValue} loading={loading} errors={errors} />

              {/* Module Image */}
              <div className="px-4 w-full mb-4">
                <ModulePhoto onChange={files => setValue('coverPhoto', files[0])} loading={loading} />
              </div>
