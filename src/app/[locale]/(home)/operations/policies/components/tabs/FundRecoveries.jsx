...
export default function FundRecoveriesByPolicyId({ policyId }) {
  ...
  const { data, isLoading } = useFundRecoveries({
    config,
    filters,
  });
  ...
  if (!data || data?.items?.length === 0) {
    return <ReceiptEmpty type="Rescate de Fondos registrados" />;
  }
            ...
            {data &&
              ...
              data?.items?.map((claim, index) => (
                <tr key={index}>
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-400 uppercase text-center">
                    {polizaFundRecoveryStatus?.[claim.status] ??
