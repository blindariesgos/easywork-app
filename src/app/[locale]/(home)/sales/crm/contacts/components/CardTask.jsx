		...
		<div className="bg-white px-4 py-3 rounded-lg w-full">
			<div className="flex justify-between items-center">
				<div className="flex gap-2 items-center">
					<p className="text-xs text-primary font-medium">{t('tools:tasks:task')}</p>
			...
			<div className="flex gap-4 md:gap-8 mt-3">
				...
				<div className="flex flex-col gap-2 w-full">
					<div className="flex gap-x-4 items-center">
						<p className="text-sm text-black font-medium">{t('tools:tasks:panel:date')}:</p>
						<p className="text-xs text-black font-normal">Lun, En 10, 8:00pm</p>
					</div>
					<div className="flex gap-x-4 items-center">
						<p className="text-sm text-black font-medium">{t('tools:tasks:panel:title')}:</p>
						<p className="text-xs text-black font-normal">Seguimiento póliza</p>
					</div>
					<div className="flex gap-x-4 items-center">
						<p className="text-sm text-black font-medium">{t('tools:tasks:panel:responsible')}:</p>
					...
					<div className="p-2 bg-yellow-50 rounded-md flex gap-2 w-full justify-between items-center">
						{!openEdit ? (
                            ...
                            <div className="border rounded-md h-28">{TextEditorComponent()}</div>
						)}
						<div className="flex gap-1 items-center">
							...
							<div className="cursor-pointer" onClick={() => setOpenEdit(false)}>
					...
					<div className="flex justify-end mt-0 gap-2 items-center">
						<div className="cursor-pointer">
							...
							>
								<div className="border rounded-md w-full h-28">{TextEditorComponent()}</div>
							</IconDropdown>
						</div>
						<IconDropdown
							icon={<EllipsisHorizontalIcon className="h-4 w-4 text-black" aria-hidden="true" />}
							options={options}
							width="w-[100px]"
						/>
					</div>
