      ...
      <Combobox as="div" value={selectedOption} onChange={setSelectedOption}>
        ...
        <div className="relative mt-2">
          {options && options.length > 0 && (
            <Combobox.Options className="absolute z-10 bottom-2 mb-8 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg focus:outline-none sm:text-sm focus:ring-0 ring-0">
              {options.map((option) => (
                <Combobox.Option
                  key={option.id}
                  value={option}
