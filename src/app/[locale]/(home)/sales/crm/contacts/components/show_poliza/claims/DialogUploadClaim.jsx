...
export default function DialogUploadClaim({ isOpen, setIsOpen, contactID }) {
    ...
    const schema = yup.object().shape({
        ...
        case: yup.string().required(t('common:validations:required')),
		...
		>
			<DialogPanel>
				<form onSubmit={handleSubmit(handleSubmitData)} className="flex w-full flex-col gap-y-3">
					<h1 className="text-center text-lg font-medium">{t('contacts:edit:policies:claims:upload')}</h1>
					<label
						type="button"
						htmlFor="filePdf"
					...
					<div className="grid sm:grid-cols-2 grid-cols-1 gap-2 w-full ">               
                        <TextInput
                            label={t('contacts:edit:policies:claims:table:claim')}
                            error={errors.claim}
                            register={register}
                            name="claim"
                            border
                        />                      
                        <TextInput
                            label={t('contacts:edit:policies:claims:table:case')}
                            error={errors.case}
                            register={register}
                            name="case"
                            border
                        />         
                        <SelectInput
							label={t('contacts:edit:policies:claims:table:status')}
                            name="status"
                            options={status}
                            border
                        ...
                        <Controller
                            render={({ field: { value, onChange, ref, onBlur } }) => {
                                return (
                                    <InputDate
                                        label={t('contacts:edit:policies:claims:table:date')}
                                        value={value}
                                        onChange={onChange}
                                        onBlur={onBlur}
