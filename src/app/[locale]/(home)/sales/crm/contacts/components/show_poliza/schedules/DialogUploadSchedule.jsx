...
export default function DialogUploadSchedule({ isOpen, setIsOpen, contactID }) {
    const { t } = useTranslation();
    const [sending, setSending] = useState(false);

    const schema = yup.object().shape({
        file: yup.mixed().required(t('common:validations:required')),
        schedule: yup.string().required(t('common:validations:required')),
		...
		>
			<DialogPanel>
				<form onSubmit={handleSubmit(handleSubmitData)} className="flex w-full flex-col gap-y-3">
					<h1 className="text-center text-lg font-medium">{t('contacts:edit:policies:schedules:upload')}</h1>
					<label
						type="button"
						htmlFor="filePdf"
					...
					<div className="grid sm:grid-cols-2 grid-cols-1 gap-2 w-full ">               
                        <TextInput
                            label={t('contacts:edit:policies:schedules:table:schedule')}
                        ...
                        />                 
                        <SelectInput
							label={t('contacts:edit:policies:schedules:table:status')}
                            name="status"
                            options={status}
                            border
                        ...
                        <Controller
                            render={({ field: { value, onChange, ref, onBlur } }) => {
                                return (
                                    <InputDate
                                        label={t('contacts:edit:policies:schedules:table:date')}
                                        value={value}
                                        onChange={onChange}
                                        onBlur={onBlur}
