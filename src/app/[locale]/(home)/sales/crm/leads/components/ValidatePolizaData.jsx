...
const ValidatePolizaData = ({ policy, isOpen, setIsOpen }) => {
  ...
  useEffect(() => {
    if (policy?.contact?.id) {
      ...
      setValue("newContact", {
        ...policy?.contact,
        name: policy?.contact?.fullName,
      });
    }
    if (policy?.poliza) setValue("poliza", policy?.poliza);
    if (policy?.contact?.typePerson)
    ...
    if (policy?.importePagar)
      setValue("importePagar", policy?.importePagar?.toFixed(2));
    if (policy?.primaNeta) setValue("primaNeta", policy?.primaNeta?.toFixed(2));
    if (policy?.primaNeta)
      setValue("derechoPoliza", policy?.derechoPoliza?.toFixed(2));
    if (policy?.iva) setValue("iva", policy?.iva?.toFixed(2));
    if (policy?.recargoFraccionado)
      setValue("recargoFraccionado", policy?.recargoFraccionado?.toFixed(2));
    ...
    setValue("polizaFileId", policy?.polizaFileId);
    setValue("status", policy?.status);
    setValue("metadata", policy?.metadata);
    if (policy?.relatedContacts && policy?.relatedContacts.length > 0) {
      setValue("relatedContacts", policy?.relatedContacts);
    }
  }, [policy]);
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
        ...
        <DialogPanel className="w-full max-w-md transform rounded-2xl bg-gray-100 p-4  shadow-xl transition-all">
          <DialogTitle className="font-bold">Validar informacion</DialogTitle>
          <Description>
                ...
                <div className="bg-gray-100 rounded-md p-2">
                  <div className="px-8 pt-4 grid grid-cols-1 gap-4">
                    <Fragment>
                      ...
                      <SelectInput
                        label={t(
                          "operations:policies:general:payment-frequency"
                        )}
                      ...
                      <div>
                        ...
                        >
                          ...
                          registros. ¿Ya está registrado? Si es así,
                          selecciónalo. Si no, crearemos uno nuevo con la
