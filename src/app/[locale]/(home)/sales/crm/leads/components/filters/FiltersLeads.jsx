...
const FiltersLead = () => {
	...
	const [ leads, setLeads ] = useState([
		{
			id: 1,
			name: t('leads:filters:proccess'),
			selected: false
		},
		{
			id: 2,
			name: t('leads:filters:closed'),
			selected: false
		},
		{
			id: 3,
			name: t('leads:filters:my-leads'),
			selected: false
		}
	]);

	const handleSelected = (id) => {
		const updateSelection = leads.map((cont) => {
			return cont.id === id ? { ...cont, selected: !cont.selected } : { ...cont, selected: false };
		});;
		setLeads(updateSelection);
	};
					...
					<div className="p-4">
						<div className="flex gap-4 flex-col sm:flex-row">
							<div className="bg-gray-150 flex flex-col w-full sm:w-40 px-4 py-2 rounded-md relative">
								<p className="text-xs text-gray-60 text-center">{t('leads:filters:name')}</p>
								<div className="mt-4 flex flex-col gap-2 mb-14">
									{leads &&
										leads.map((cont, index) => (
											<div
												key={index}
								...
								<div className="absolute bottom-2">
									<div className="flex gap-2 cursor-pointer items-center">
										...
										<p className="text-xs uppercase text-gray-60">{t('leads:filters:save')}</p>
									</div>
								</div>
							</div>
                            <FormFilters/>
