...
import { TbBrandGoogleDrive, TbBrandOffice, TbAppWindow, TbCloudUpload } from 'react-icons/tb';
  ...
  return (
    <Fragment>
      <Menu>
        ...
        >
          <MenuItems anchor="bottom start" className="divide-y z-10 divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="py-1">
            ...
            <div className="py-1">
              <MenuItem disabled>
                {({ active }) => (
                  <a
                    href="#"
                    className={clsx(active ? 'bg-easy-600 text-white' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm data-[disabled]:cursor-no-drop data-[disabled]:opacity-50')}
                  >
                    <TbCloudUpload className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
              ...
              <MenuItem disabled>
                {({ active }) => (
                  <a
                    ...
                    className={clsx(active ? 'bg-easy-600 text-white' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm data-[disabled]:cursor-no-drop data-[disabled]:opacity-50')}
                  >
                    <TbBrandGoogleDrive className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
            ...
            <div className="py-1">
              <MenuItem disabled>
                {({ active }) => (
                  <a
                    href="#"
                    className={clsx(active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'group flex items-center px-4 py-2 text-sm data-[disabled]:cursor-no-drop data-[disabled]:opacity-50')}
                  >
                    <TbBrandOffice className="mr-3 h-5 w-5 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel transition className="w-full max-w-md rounded-xl bg-white p-6 backdrop-blur-2xl duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0">
              ...
              <form className="py-10 px-2 bg-[#F2F6F7] mt-4" onSubmit={e => e.preventDefault()}>
                <TextInput type="text" label={'Nombre de carpeta'} name="responsible" value={folderName} onChangeCustom={handleChangeFolderName} />
              </form>
              <div className="mt-4 flex justify-center gap-4">
                <Button buttonStyle="primary" className="inline-flex items-center gap-2 rounded-md py-1.5 px-3 text-sm/6 font-semibold " onclick={handleChangeAddFolder} label="Crear" />
                <Button buttonStyle="secondary" className="inline-flex items-center gap-2 rounded-md py-1.5 px-3 text-sm/6 font-semibold " onclick={() => setIsOpenAdd(false)} label="Cerrar" />
              </div>
            </DialogPanel>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel transition className="w-full max-w-md rounded-xl bg-white p-6 backdrop-blur-2xl duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0">
              ...
              <div className="mt-4 flex justify-center gap-4">
                <Button buttonStyle="primary" className="inline-flex items-center gap-2 rounded-md py-1.5 px-3 text-sm/6 font-semibold " onclick={handleSaveFiles} label="Agregar" />
                <Button buttonStyle="secondary" className="inline-flex items-center gap-2 rounded-md py-1.5 px-3 text-sm/6 font-semibold " onclick={() => setIsOpenAddFile(false)} label="Cerrar" />
              </div>
            </DialogPanel>
          </div>
        </div>
