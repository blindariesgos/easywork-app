...
import { putTaskRestart, convertToSubtaskOf, toggleTaskTracking } from '@/src/lib/apis';
...
export default function ButtonMore({ setOpenEdit, data, setIsDelegating, canEdit, isCreator }) {
  ...
  const handleTracking = async () => {
    ...
    toast.success(`Se ${!!data?.timeTrackingEnabled ? 'desactivo' : 'activo'} el seguimiento de tiempo`);
    setLoading(false);
  };
  ...
  const [options, setOptions] = useState([
    ...
    {
      id: 5,
      name: !!data?.timeTrackingEnabled ? 'Desactivar seguimiento' : 'Activar seguimiento',
      icon: TfiTimer,
      onclick: handleTracking,
      hidden: !isCreator || !!data?.currentTrackingStartTime,
    },
  ]);
  ...
  const handleSelectParentTask = async parent => {
    ...
    setLoading(true);
    const response = await convertToSubtaskOf(data.id, parent.id).catch(error => ({ hasError: true, ...error }));
    if (response.hasError) {
      toast.error('Se ha producido un error al convertir a subtarea, inténtelo de nuevo más tarde.');
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    if (data && !data.isCompleted && !options.find(option => option.name === t('tools:tasks:edit:edit')) && canEdit) {
      setOptions([
        ...options,
        {
          id: 4,
  ...
  return (
    <Fragment>
      ...
      <Menu as="div" className="relative inline-block text-left">
        ...
        >
          <MenuItems anchor="bottom start" className=" mt-2 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-50 ">
            <div className="px-1 py-1 ">
              {options
                ...
                .map((opt, index) => (
                  <MenuItem key={index}>
                    {() => (
                      <button onClick={opt.onclick} className={`group flex w-full text-gray-400 data-[focus]:bg-primary data-[focus]:text-white items-center gap-1 rounded-md px-2 py-2 text-xs`}>
      ...
      <SelectTasks subtitle={'Seleccionar tarea padre'} taskId={data.id} handleSelect={handleSelectParentTask} isOpen={isOpenParentTaskModal} setIsOpen={setIsOpenParentTaskModal} />
    </Fragment>
  );
}
