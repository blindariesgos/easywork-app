  ...
  return (
    <div className="flex flex-wrap w-full">
      <div className="md:ml-[152px] gap-2 sm:gap-6 grid sm:grid-cols-3 grid-cols-1 w-full">
        <InputDateV2 label={t('tools:tasks:new:init-task')} value={startDate} onChange={setStartDate} icon={<FaCalendarDays className="h-4 w-4 text-primary" />} time watch={watch} />
        <div className="flex flex-col gap-1 w-full">
          <TextInput type="number" label={t('tools:tasks:new:duration')} onChangeCustom={e => updateDuration(e.target.value, durationUnit)} value={value} />
          <div className="flex gap-2">
            {['days', 'hours', 'minutes'].map((time, index) => (
              <div key={index} className={`cursor-pointer hover:text-primary hover:font-semibold ${time === durationUnit ? 'text-primary font-semibold' : ''}`} onClick={() => onChangeTime(time)}>
                <p className="text-xs">{t(`tools:tasks:new:${time}`)}</p>
              </div>
            ))}
          </div>
        </div>
        <InputDateV2 label={t('tools:tasks:new:end')} value={endDate} onChange={setEndDate} icon={<FaCalendarDays className="h-4 w-4 text-primary" />} time watch={watch} />
      </div>
    </div>
  );
};
