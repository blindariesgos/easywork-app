...
export default function TabsTask({ data }) {
  ...
  useEffect(() => {
    const timer = calculateElapsedTime(data?.totalTimeSpent ? data?.totalTimeSpent * 1000 : 0, moment().format());
    setCategories({
      comments: {
              ...
              <Tab
                ...
                className={({ selected }) =>
                  classNames(
                    ...
                    selected ? `${category === 'comments' ? 'bg-white' : 'bg-gray-100'} text-black shadow` : 'text-black hover:bg-white/[0.12] hover:text-white bg-gray-300'
                  )
                }
              >
                {t(`tools:tasks:edit:${category}`)}
                <div className="text-xs p-1 bg-gray-200 rounded-md">{categories[category].qty}</div>
              </Tab>
            ))}
        </TabList>
        <TabPanels className="w-full">
          {categories &&
            Object.values(categories).map((categ, idx) => (
              <TabPanel key={idx} className={classNames(`rounded-lg ${idx === 0 ? 'bg-white' : 'bg-gray-100'}`, 'focus:outline-none focus:ring-0')}>
