...
export default function TaskEntiy({ task, lists, entityKey, label, field, getFilteredUsers, updateTaskBody, disabled }) {
    ...
    await handleSubmit(async data => {
      ...
      try {
        ...
        if (response?.hasError) {
          toast.error(response?.error?.message ?? 'Ocurrio un error al editar la tarea, intente mas tarde');
          setIsLoading(false);
          return;
        }
  ...
  return (
    <div className="relative mb-4" ref={containerRef}>
      <div className="flex justify-between border-b-[1px] border-slate-300/40 pt-2 pb-1">
        ...
        {!disabled && (
          <p className="text-xs text-slate-400 cursor-pointer hover:text-slate-500" onClick={isLoading ? () => {} : handleEditClick}>
      ...
      {task[entityKey]?.length > 0 &&
        task[entityKey].map((entity, index) => (
          <div className="flex gap-2 items-center mt-3" key={index} onMouseEnter={() => setIsHovering(index)} onMouseLeave={() => setIsHovering(false)}>
            <Image className="h-8 w-8 rounded-full object-cover" width={50} height={50} src={entity?.avatar || '/img/avatar.svg'} alt="" objectFit="cover" />
            <p className="font-semibold text-blue-800 text-sm">{entity?.name || entity?.username}</p>
            {isHovering === index && <FaTimes className="ml-2 text-indigo-500 cursor-pointer" onClick={() => handleRemoveEntity(entity.id)} />}
          </div>
        ))}
      <Transition
        show={isEditing}
      ...
      >
        <div className="absolute w-full z-50 bg-white shadow-lg rounded-md" style={{ top: position.top + 20, left: 'auto', right: 'auto' }}>
          <DropdownSelect {...field} options={filteredUsers} getValues={getValues} setValue={handleChangeEntity} name="entities" error={errors.entities} isOpen={isEditing} setIsOpen={setIsEditing} />
        </div>
      </Transition>
    </div>
  );
