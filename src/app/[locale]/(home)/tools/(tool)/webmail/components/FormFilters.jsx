...
const FormFilters = () => {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormFilters)} className="grid grid-cols-1 gap-2 sm:w-96 w-72 px-2">
      <UserSelectAsync label={t('contacts:filters:responsible')} name="responsible" setValue={setValue} />
      {/* <Controller
				name="responsible"
				control={control}
				defaultValue={[]}
      ...
      <div className="flex gap-2 items-end">
        <div className={`${watch('date')?.date ? 'w-2/5' : 'w-full'}`}>
          <SelectInput label={t('contacts:filters:created')} name="date" options={createdDate} setValue={setValue} object={true} />
        </div>
        ...
        {watch('date')?.date === 'exactDate' && (
          <div className="w-3/5 flex gap-2">
            <DatePicker selected={watch('exactDate')} onChange={date => setValue('exactDate', date)} className="w-full border-none drop-shadow-sm h-9 rounded-md text-sm focus:ring-0" />
          </div>
        )}
      ...
      {/* <Controller
			...
			/> */}
      <UserSelectAsync label={t('contacts:filters:created-by')} name="createdby" selectedOption={null} setValue={setValue} />
      {fields.map((field, index) => {
        return (
          <div key={field.id}>
            {field.type === 'input' && <TextInput label={field.name} type="text" name={`fields[${index}].value`} register={register} />}
            {field.type === 'select' && <SelectInput label={field.name} name={`fields[${index}].value`} options={field.options} setValue={setValue} />}
          </div>
        );
      })}
      ...
      <div className="flex gap-2 justify-center mt-4">
        <Button type="submit" label={t('contacts:filters:search')} buttonStyle="primary" className="py-1 px-3" iconLeft={<FaMagnifyingGlass className="h-4 w-4 text-white" />} />
        <Button
          type="button"
