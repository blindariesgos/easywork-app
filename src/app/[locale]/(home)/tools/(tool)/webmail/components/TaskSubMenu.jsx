...
export default function TaskSubMenu({ selectedFolder, totalUnreadByPage, setDMails, getAxiosMails }) {
  ...
  async function updateLabelId(array, label) {
    setSelectedEmails([]);
    await axios.post(`${process.env.NEXT_PUBLIC_API_THIRDPARTY}/gmail/updatelabel/${label}/${session?.data.user.sub}/${selectOauth?.id}`, { data: array });

    const updatedMails = await getAxiosMails(selectedFolder);
    setDMails(prevMails => prevMails.map(mail => updatedMails.find(updatedMail => updatedMail.email.googleId === mail.email.googleId) || mail));
  ...
  return (
    <div className="flex gap-6 text-sm text-easywork-main ml-9">
      <div>
        <p>
          {' '}
          <span className="bg-white rounded-full p-1">{totalUnreadByPage()}</span> Correos electrónicos
        </p>
      </div>
      <div>
        <p
        ...
        >
          <span className="bg-white rounded-full p-1">{selectedEmails.length}</span> Marcar como no leídos
        </p>
      </div>
      <div>
        <p
