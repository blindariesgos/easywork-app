...
export default function TaskCreate({ edit }) {
	...
	const [openOptions, setOpenOptions] = useState({
		...
		obserbers: edit?.observers?.length > 0 ? true : false,
		time: edit?.deadline ? true : false,
		options: (edit?.responsibleCanChangeDate || edit?.requireRevision)  ? true : false,
		more: false
	})
	...
	useEffect(() => {
		if (edit){
			...
			if ( edit?.requireRevision ) { optionsSelected.push(
				{
					id: 2,
	...
	useEffect(() => {
		if (session) setValue("createdBy", lists?.users.filter(user => user.id === session.user.user?.id))
	...
	return (
		...
		{loading && <LoaderSpinner />}
			<div className={`flex flex-col ${edit ? "h-full" : "h-screen"} relative w-full ${!edit && "overflow-y-auto"}`}>
