      ...
      <div className='flex justify-center items-center h-full py-8'>
        <div className='sm:w-1/2 w-full bg-white rounded-xl drop-shadow-md px-8 py-10 flex items-center flex-col gap-4'>
          ...
          <ul className={clsx(
            ...
            "sm:grid-cols-2": options?.children?.length % 2 == 0 && options?.children?.length <= 4,
            "sm:grid-cols-3": (options?.children?.length % 3 == 0 || options?.children?.length > 4) && options?.children?.length != 1,
