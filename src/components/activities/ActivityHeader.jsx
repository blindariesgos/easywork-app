...
export default function ActivityHeader({ entityId, update, crmType = 'contact', contactType = 'fisica', disabled }) {
  ...
  const tabs = [
    {
      ...
      onClick: () => router.push(`/tools/tasks/task?show=true&prev=${crmType}&prev_id=${entityId}`),
    },
    {
    ...
    },
    {
      ...
      onClick: () => router.push(`/tools/calendar/addEvent?show=true&prev=${crmType}&prev_id=${entityId}`),
    ...
    },
    {
      ...
      onClick: () => router.push(`/agents-management/meetings-and-sessions/individuals/meet?show=true&prev=agent-meet&prev_id=${entityId}`),
    },
  ];
  return (
    <>
      <div className="bg-white px-2 md:px-4 rounded-lg w-full shadow-sm">
                    ...
                    <div key={tab.name}>
                      {tab.children ? (
                        <div key={tab.name} className="py-4 -mt-4">
                          <AppointmentMenu options={tab.children} label={tab.name} />
                        </div>
                      ) : (
                        <div
                          key={tab.name}
                          className={clsx('border-transparent text-gray-400 hover:border-gray-300 hover:text-gray-700', 'whitespace-nowrap py-2 px-1 text-sm font-medium uppercase cursor-pointer')}
        ...
        <div className="py-2">
          {isShowAddComment ? (
            <AddComment entityId={entityId} crmType={crmType} updateActivities={update} close={() => setIsShowAddComment(false)} />
          ) : (
            <input
              type="text"
              name="todo"
  ...
  return (
    <div>
      ...
      <div className="pt-2 flex justify-end gap-2">
        <Button label={loading ? 'Loading...' : 'Guardar'} className="text-sm px-2 py-1" buttonStyle="primary" disabled={loading} onclick={handleAdd} />
        <Button label="Cancelar" className="text-sm px-2 py-1" buttonStyle="secondary" disabled={loading} onclick={close} />
      </div>
    </div>
  );
};
