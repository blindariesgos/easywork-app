...
import { addContactDocument, addLeadPolicy, addReceiptDocument } from '@/src/lib/apis';
import { addLeadDocument } from '@/src/lib/apis';

const useAddDocument = ({ fileName, documentType, id, cmrType, onFinished, update }) => {
  const endpoints = {
  ...
  const handleFormSubmit = async file => {
    ...
    try {
      ...
      if (response?.client?.fullName) {
        toast.success(`Se cargo con éxito póliza con contratante ${response.client.fullName}`);
      } else {
