...
export const getAllRefunds = async ({ filters = {}, userId = '', config = {} }) => {
...
export const getAllSchedules = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
export const addAgentDocument = async (agentId, category, body) => {
  const response = await axios({ contentType: 'multipart/form-data' })
    .post(`/agent-management/agents/upload/${agentId}?category=${category}`, body)
    .catch(error => ({ hasError: true, ...error }));
  return response;
};
...
export const deleteTrackingEntry = async (taskId, entryId) => {
  const response = await axios()
    .delete(`/tools/tasks/time-tracking/${taskId}/time-tracking/entries/${entryId}`)
    .catch(error => ({ ...error, hasError: true }));
  return response;
};
...
export const updateTrackingEntry = async (taskId, entryId, body) => {
  const response = await axios()
    .patch(`/tools/tasks/time-tracking/${taskId}/time-tracking/entries/${entryId}`, body)
    .catch(error => ({ ...error, hasError: true }));
  return response;
};
...
//#region RECRUITMENT
export const getAllRecruitments = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
//#region CONNECTIONS
export const getAllConnections = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
export const getAllClaims = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
export const addFundRecoveryDocument = async (fundId, category, body) => {
  const response = await axios({ contentType: 'multipart/form-data' })
    .post(`/operations/fund-recoveries/upload/${fundId}?category=${category}`, body)
    .catch(error => ({ ...error, hasError: true }));
  return response;
};
...
export const getAllFundRecoveries = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
export const deleteFileMeetById = async (meetingId, body) => {
  const response = await axios()
    ...
    .catch(error => ({ hasError: true, error }));
  revalidatePath(`/agents-management/meetings-and-sessions/teams/meet/${meetingId}`, 'page');
  return response;
};
...
export const postMeetComment = async body => {
  const response = await axios().post(`/agent-management/meetings/comments`, body);
  return response;
};
...
export const deleteMeetComment = async commentId => {
  const response = await axios().delete(`/agent-management/meetings/comments/${commentId}`);
  return response;
};
...
export const putMeetComment = async (commentId, body) => {
  const response = await axios().put(`/agent-management/meetings/comments/${commentId}`, body);
  return response;
};
...
export const deleteTokenGoogle = async (userId, oauthId, refreshtoken, fromCalendar) => {
  const response = await axios().delete(`/oauth/${userId}/${oauthId}?refreshtoken=${refreshtoken}&fromCalendar=${fromCalendar}`);
  return response;
};
...
export const getAllTasks = async ({ filters = {}, userId = '', config = {} }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
...
export const getAllPolicies = async ({ filters = {}, config = {}, userId = '' }) => {
  const queries = getQueries(filters, userId);
  const configParams = Object.keys(config)
    .map(key => `${key}=${config[key]}`)
    .join('&');
