...
export const handleApiError = async (error, errorsDuplicated) => {
  ...
  if (!errorsDuplicated?.current) {
    switch (errorObject?.statusCode) {
      case 403:
        isServerSide ? logger.error('Forbidden Access (403):', errorObject) : toastError(errorObject?.message || errorObject);
        logout();
        break;
      case 401:
        isServerSide ? logger.error('Unauthorized (401):', errorObject) : logout();
        break;
      default:
        isServerSide ? logger.error('API Error:', errorObject) : toastError(errorObject?.message || 'An error occurred');
    }
